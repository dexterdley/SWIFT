...
from swift.trainers.trainer_factory_vord import TrainerFactoryVORD
...
class SwiftSftVORD(SwiftPipeline, TunerMixin):
    ...
    def run(self):
        ...
        trainer_cls = TrainerFactoryVORD.get_trainer_cls(args)
        trainer = trainer_cls(
            model=self.model,
...
def sft_vord_main(args: Union[List[str], TrainArguments, None] = None):
    return SwiftSftVORD(args).main()
